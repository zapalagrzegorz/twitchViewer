...
var engine = {
	...
	getUsersData: function getUsersData(single) {
		var self = this;
   ...
   * ********************************************/
		...
		function getSearchData(single) {
			...
			$.when.apply($, promises).done(handleStreamResponses);
		}
		...
		function handleStreamResponses() {
			...
			if (promises.length != 0) {
				$.when.apply($, promises).done(handleSuccessUsers);
			}
		}
  ...
  * *****************************************************/
		...
		function produceOutput(typeOfUser, user) {
			for (var i in user) {
				...
				//  end of darken
				userHTML += '<img class="img" src="' + pic + '"></div></div><div class="channelDescription">' + '<h3><a href="https://www.twitch.tv/' + mainDesc + '" target="_blank">' + mainDesc + '</a></h3>';

				if (typeOfUser == 'streaming') {
					userHTML += '<div class="row"><div class="col-10">' + '<h4><a href="https://www.twitch.tv/' + user[i].channel.name + '" target="_blank">' + user[i].channel.status + '</a></h4></div>' + '<div class="col-2" style="text-align: center">' + '<i class="fa fa-2x fa-user-circle" aria-hidden="true"></i><p><span id="#online">' + user[i].viewers + '</span> online</p></div></div>';
				} else if (typeOfUser == 'nostreaming') {
					userHTML += '<div class="row"><div class="col-12">' + '<p class="user-bio">' + user[i].bio + '</p></div></div>';
				}

				userHTML += '<div class="row"><div class="col-2" style="padding-right:0;">' + '<a href="https://www.twitch.tv/' + name + '" target="_blank">' + '<img class="img" src="' + logo + '"></a></div>' + '<div class="col-10" id="lastStatus' + mainDesc + '">' + '<p>followers: ' + followers + '</p>' + '<p>Launched: ' + launchedAt + '</p>' + '</div></div></div></div></article>';
